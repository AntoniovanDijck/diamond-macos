...
class Denoiser(nn.Module):
    ...
    def forward(self, batch: Batch) -> LossAndLogs:
        ...
        if self.is_upsampler:
            ...
            low_res = F.interpolate(batch.obs.view(b * t, c, h, w), scale_factor=self.cfg.upsampling_factor, mode="bicubic").view(b, t, c, H, W)
            assert all_obs.shape == low_res.shape
        else:
            all_obs = batch.obs.clone()

        loss = 0
        sigma_training = self.sample_sigma_training

        # Precompute sigma for all steps
        sigma = sigma_training(b * seq_length, device).view(b, seq_length, 1, 1, 1)
        if self.cfg.noise_previous_obs:
            sigma_cond = sigma_training(b * seq_length, device).view(b, seq_length, 1, 1, 1)
            noise_prev = self.apply_noise(all_obs[:, :seq_length + n].reshape(b, -1, H, W), sigma_cond, self.cfg.sigma_offset_noise)
        else:
            sigma_cond = None

        noisy_obs = self.apply_noise(all_obs[:, n:n + seq_length], sigma, self.cfg.sigma_offset_noise)

        # Compute conditioners in batch
        cs = self.compute_conditioners(sigma, sigma_cond)

        # Reshape previous observations and actions
        prev_obs = all_obs[:, :seq_length + n].reshape(b, n * c, H, W)
        if not self.is_upsampler:
            prev_act = batch.act[:, :seq_length + n]
        else:
            prev_act = None

        # Compute model outputs in batch
        model_output = self.compute_model_output(noisy_obs, prev_obs, prev_act, cs)

        # Compute targets and loss
        target = (all_obs[:, n:n + seq_length] - cs.c_skip * noisy_obs) / cs.c_out
        mask = batch.mask_padding[:, n:n + seq_length].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        loss = F.mse_loss(model_output[mask], target[mask])

        # Wrap model outputs
        denoised = self.wrap_model_output(noisy_obs, model_output, cs)
        all_obs[:, n:n + seq_length] = denoised
